        ...
        {isLoading ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-slate-200 dark:border-slate-800 text-[11px] font-bold uppercase tracking-wider text-slate-800 dark:text-slate-200 bg-slate-50 dark:bg-slate-950/60">
                  <th className="py-3.5 px-4 w-36 text-center">ICD-11 Code</th>
                  <th className="py-3.5 px-4 min-w-[240px] text-center">Disease / Entity Title</th>
                  <th className="py-3.5 px-4 w-24 text-center">System</th>
                  <th className="py-3.5 px-4 w-36 text-center">Entity Type</th>
                  <th className="py-3.5 px-4 w-20 text-center">Ch #</th>
                  <th className="py-3.5 px-4 min-w-[280px] text-center">Chapter Name</th>
                  <th className="py-3.5 px-4 w-28 text-center">WHO Profile</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-100 dark:divide-slate-800/60 text-xs">
                {diseases.map((d, idx) => {
                  const isCopied = copiedCode === d.code;
                  const chClean = String(d.chapterNumber || '').replace(/^0+/, '');
                  const isSymptom = chClean === '21' || (d.chapter || '').toLowerCase().includes('symptom');
                  const isDisease = !isSymptom && (parseInt(chClean, 10) >= 1 && parseInt(chClean, 10) <= 20);
                  ...
                  return (
                    ...
                    >
                      {/* Code with 1-click Copy & Official WHO Browser Link */}
                      <td className="py-3.5 px-4 align-middle text-center">
                        <div className="flex items-center justify-center gap-1.5">
                          ...
                          {d.browserUrl && (
                            <a
                              ...
                              target="_blank"
                              rel="noopener noreferrer"
                              className="p-1 rounded-md text-slate-400 hover:text-blue-600 dark:hover:text-blue-400 hover:bg-blue-50 dark:hover:bg-slate-800 transition-colors opacity-0 group-hover:opacity-100 cursor-pointer"
                              title="Open in Official WHO ICD-11 Browser"
                            >
                        ...
                        {isCopied && (
                          <div className="text-[10px] text-emerald-500 font-bold mt-0.5 text-center">Copied!</div>
                        )}
                      </td>

                      {/* Display / Title */}
                      <td className="py-3.5 px-4 align-middle text-center">
                        <div className="font-bold text-slate-900 dark:text-white text-sm leading-snug text-center">
                          {d.display}
                        </div>
                      </td>
                      ...
                      <td className="py-3.5 px-4 align-middle text-center">
                        {d.system && (
                          <span className="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-mono bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400">
                            {d.system}
                          </span>
                        )}
                      </td>
                      ...
                      <td className="py-3.5 px-4 align-middle text-center">
                        {isSymptom ? (
                          <span className="inline-flex items-center justify-center px-2 py-0.5 rounded text-[10px] font-bold bg-purple-50 text-purple-700 dark:bg-purple-950/80 dark:text-purple-300 border border-purple-200 dark:border-purple-800/60 whitespace-nowrap">
                            Sign / Symptom
                          </span>
                        ) : isDisease ? (
                          <span className="inline-flex items-center justify-center px-2 py-0.5 rounded text-[10px] font-bold bg-blue-50 text-blue-700 dark:bg-blue-950/80 dark:text-blue-300 border border-blue-200 dark:border-blue-800/60 whitespace-nowrap">
                            Disease / Condition
                          </span>
                        ) : (
                          <span className="inline-flex items-center justify-center px-2 py-0.5 rounded text-[10px] font-medium bg-slate-100 text-slate-700 dark:bg-slate-800 dark:text-slate-300 border border-slate-200 dark:border-slate-700 whitespace-nowrap">
                            Health Concept
                          </span>
                        )}
                      </td>
                      ...
                      <td className="py-3.5 px-4 align-middle text-center">
                        {d.chapterNumber && (
                          <span className="text-[10px] text-slate-500 font-bold bg-slate-100 dark:bg-slate-800 px-2 py-0.5 rounded whitespace-nowrap">
                            Ch {d.chapterNumber}
                          </span>
                        )}
                      </td>
                      ...
                      {/* Chapter Name */}
                      <td className="py-3.5 px-4 align-middle text-center">
                        <span className="inline-flex items-center justify-center px-3 py-1.5 rounded-lg text-[11px] font-semibold bg-blue-50 dark:bg-blue-950/80 text-blue-700 dark:text-blue-300 border border-blue-200 dark:border-blue-800/60 leading-relaxed text-center">
                          {d.chapter}
                        </span>
                      </td>

                      {/* WHO Foundation Profile - Clean Eye Icon Button (END COLUMN) */}
                      <td className="py-3.5 px-4 align-middle text-center">
                        <button
                          type="button"
                          onClick={() => setSelectedDiseaseForModal(d)}
                          className="inline-flex items-center justify-center p-2 rounded-xl text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-blue-950/80 hover:bg-blue-600 hover:text-white dark:hover:bg-blue-500 dark:hover:text-white border border-blue-200 dark:border-blue-800 transition-all cursor-pointer shadow-xs group"
                          title={`View WHO Foundation Entity Profile for ${d.display} (Classification, Definition & Browser)`}
                        >
